...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		multiSelect: function(o, callback) {
			...
			$(this).each( function() {
				var select = $(this);
				var html = '<input type="hidden" readonly="readonly" class="multiSelect" value="" />';
				html += '<div class="multiSelectOptions" z-index: 99999;">';
				if( o.selectAll ) html += '<label class="selectAll"><input type="checkbox" class="selectAll" />' + o.selectAllText + '</label>';
				$(select).find('optgroup').each( function() {
					var optgroup = $(this);
					html += '<strong>'+optgroup.attr('label')+'</strong><br />';
					$(select).find('optgroup#opt-'+optgroup.attr('label')+' > OPTION').each( function() {
						if( $(this).val() != '' ) {
