...
class Unodor_Image_Resize
{
   ...
   {
      /*
       * f(!is_numeric($width) OR $width == 0){
       * hrow new Zend_Exception('$width must be numeric bigger then 0');
       }
       * f(!is_numeric($height) OR $height == 0){
       * hrow new Zend_Exception('$height must be numeric bigger then 0');
       }
       * this->maxSize = array(
       * width' => (intval($width) > $this->oldSize['width']) ? $this->oldSize['width'] : intval($width),
       * height' => (intval($height) > $this->oldSize['height']) ? $this->oldSize['height'] : intval($height)
       ...
       * cropX = 0;
       * cropY = 0;
       * f($this->newSize['width'] > $this->maxSize['width']){
